...
{  
  ...
  dacValue = (int)((MAX_DAC_RAW_VALUE - TriggerThreshold[channel])/(MAX_DAC_RAW_VALUE - MIN_DAC_RAW_VALUE) * 65535);  // Gamme inversée
...
/* ============================================================================ */ 
{
  ...
  dacValue = (int)((MAX_DAC_RAW_VALUE - DCOffset[channel])/(MAX_DAC_RAW_VALUE - MIN_DAC_RAW_VALUE) * 65535);  // Gamme inversée
		  ...
		  if( CurrentEvent->GrPresent[samIndex])
		    {
		      ...
		      PlotWaveform (MainPanelHandle, MAINPANEL_GRAPH, CurrentEvent->DataGroup[samIndex].DataChannel[0], CurrentEvent->DataGroup[samIndex].ChSize, VAL_FLOAT,
				    (double)1000*ADCTOVOLTS/VerticalScale, yOffset, 0, 1, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, Set_Color(samIndex*2));
